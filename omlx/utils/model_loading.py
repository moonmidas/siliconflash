...
def apply_post_load_transforms(model: Any, model_settings: Any = None) -> Any:
    ...
    if os.environ.get("DFLASH_BOUNDARY_SNAPSHOT") == "1":
        from ..patches.gated_delta_boundary_capture import (
            apply_gated_delta_boundary_capture_patch,
        )

        if apply_gated_delta_boundary_capture_patch(model):
            logger.info("GatedDeltaNet boundary capture patch applied")
