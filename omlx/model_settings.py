...
class ModelSettings:
    ...
    # DFlash (experimental: block-diffusion speculative decoding)
    dflash_enabled: bool = False
    dflash_auto_profile: bool = False
    dflash_draft_model: Optional[str] = None
    dflash_use_mlx_native_drafter: bool = False
    dflash_block_size: int = 16
    dflash_verify_kernel: str = "metal_batched_gemv"
    dflash_single_batch_only: bool = True
    dflash_report_acceptance_rate: bool = True
...
def apply_dflash_fast_model_settings(
    ...
    *,
    ...
    """Apply the tuned DFlash fast-path defaults to a model settings object."""
    current = ModelSettings.from_dict(settings.to_dict())
    current.dflash_enabled = True
    current.dflash_auto_profile = True
    current.dflash_draft_model = draft_model
    current.dflash_use_mlx_native_drafter = True
    current.temperature = 0.0
    current.top_p = 1.0
    current.top_k = 0
    current.min_p = 0.0
    current.force_sampling = False

    chat_kwargs = dict(current.chat_template_kwargs or {})
    forced = set(current.forced_ct_kwargs or [])
    if enable_thinking:
        chat_kwargs["enable_thinking"] = True
        forced.add("enable_thinking")
    else:
        chat_kwargs.pop("enable_thinking", None)
        forced.discard("enable_thinking")
    current.chat_template_kwargs = chat_kwargs or None
    current.forced_ct_kwargs = sorted(forced) or None

    current.thinking_budget_enabled = thinking_budget is not None
    current.thinking_budget_tokens = thinking_budget
    return current
