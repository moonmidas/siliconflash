            ...
            try:
                if self.scheduler.has_requests():
                    ...
                    if outputs:
                        ...
                        # With finish-only non-streaming outputs, yielding on
                        # every generation step can become pure overhead. Yield
                        # immediately on finishes, otherwise only when a streamed
                        # consumer is actually waiting or every few steps.
                        should_yield = any(req_output.finished for req_output in outputs)
                        if not should_yield:
                            should_yield = RequestOutputCollector.has_waiting_consumers()
                        if not should_yield:
                            idle_yield_counter = (idle_yield_counter + 1) % 8
                            should_yield = idle_yield_counter == 0
                        else:
                            idle_yield_counter = 0
