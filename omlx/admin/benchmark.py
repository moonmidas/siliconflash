...
def _compute_single_metrics(
    ...
    return {
        "benchmark_mode": "standard_stream",
...
def _compute_nonstream_single_metrics(
    prompt_tokens: int,
    completion_tokens: int,
    start_time: float,
    end_time: float,
    ...
    e2e_duration = end_time - start_time
    gen_tps = completion_tokens / max(e2e_duration, 1e-9)
    total_throughput = (prompt_tokens + completion_tokens) / max(e2e_duration, 1e-9)
    return {
        "benchmark_mode": "nonstream_dflash",
        "ttft_ms": None,
        "tpot_ms": None,
        "gen_tps": round(gen_tps, 1),
        "processing_tps": None,
...
def _should_use_nonstream_single_request_benchmark(engine: Any, max_tokens: int) -> bool:
    runtime = getattr(engine, "_dflash_runtime", None)
    if runtime is None:
        return False
    try:
        from ..request import SamplingParams

        supported, _reason = runtime.request_is_supported(
            stream=False,
...
async def _run_single_test(
    ...
    use_nonstream_dflash = _should_use_nonstream_single_request_benchmark(
        engine, max_tokens=max_tokens
    )

    start_time = time.perf_counter()
    if use_nonstream_dflash:
        ...
        completion_tokens = output.completion_tokens if output else 0
        cached_tokens = output.cached_tokens if output else 0
    else:
    ...
    if use_nonstream_dflash:
        return _compute_nonstream_single_metrics(
            prompt_tokens=prompt_tokens,
            completion_tokens=completion_tokens,
...
async def _upload_to_omlx_ai(run: BenchmarkRun, engine_pool: Any) -> None:
    ...
    single_results = [
        ...
        and r.get("processing_tps") is not None
...
async def run_benchmark(run: BenchmarkRun, engine_pool: Any) -> None:
    ...
    try:
        ...
        # one-time overhead and shows artificially low throughput.
        await _send_event(run, {
            "type": "progress",
            "phase": "warmup",
        ...
        if _should_use_nonstream_single_request_benchmark(engine, max_tokens=8):
