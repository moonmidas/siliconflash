...
## SiliconFlash Research Artifacts

This repo is prepared to share the **oMLX code changes** and the **autoresearch record** without uploading model artifacts.

Included in git:
- oMLX runtime/code changes (mainly under `omlx/`)
- `autoresearch.md` (session scope + methodology)
- `autoresearch.jsonl` (full experiment log: kept/discarded/crashed runs)
- `autoresearch.ideas.md` (deferred ideas backlog)

Intentionally excluded from git:
- local model folders (`models/`, `mlx_models/`)
- local benchmark workspaces and logs (`.omlx-*`)
- machine-local agent/cache files

### Model references used for the current high-throughput DFlash lane

- **Target model**: `Qwen3.6-35B-A3B-MLX-8bit`
- **Draft model**: `z-lab/Qwen3.6-35B-A3B-DFlash`

Use local paths via env vars (no machine-specific absolute paths in docs):
