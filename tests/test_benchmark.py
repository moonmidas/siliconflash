...
from omlx.admin.benchmark import (
    ...
    _generate_prompt,
    _run_single_test,
    _should_use_nonstream_single_request_benchmark,
    cleanup_old_runs,
    create_run,
    get_run,
)
...
class TestSingleRequestBenchmarkMode:
    def test_uses_nonstream_mode_when_dflash_runtime_supports_request(self):
        runtime = MagicMock()
        runtime.request_is_supported.return_value = (True, None)
        engine = MagicMock()
        engine._dflash_runtime = runtime

        assert _should_use_nonstream_single_request_benchmark(engine, max_tokens=128) is True

    def test_keeps_standard_mode_without_supported_dflash_runtime(self):
        engine = MagicMock()
        engine._dflash_runtime = None

        assert _should_use_nonstream_single_request_benchmark(engine, max_tokens=128) is False

    @pytest.mark.asyncio
    async def test_run_single_test_uses_nonstream_generate_for_dflash_mode(self, monkeypatch):
        perf_values = iter([10.0, 12.0])
        monkeypatch.setattr("omlx.admin.benchmark.time.perf_counter", lambda: next(perf_values))
        monkeypatch.setattr("omlx.admin.benchmark._should_use_nonstream_single_request_benchmark", lambda engine, max_tokens: True)
        ...
        assert metrics["benchmark_mode"] == "nonstream_dflash"
        assert metrics["ttft_ms"] is None
        assert metrics["tpot_ms"] is None
        assert metrics["processing_tps"] is None
        assert metrics["gen_tps"] == pytest.approx(64.0, abs=0.1)
        assert metrics["e2e_latency_s"] == pytest.approx(2.0, abs=0.001)
