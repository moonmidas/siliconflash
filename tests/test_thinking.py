...
class TestCleanOutputTextBackwardCompat:
    ...
    def test_extract_thinking_treats_unclosed_open_tag_as_reasoning(self):
        from omlx.api.thinking import extract_thinking
        thinking, content = extract_thinking("<think>reasoning only")
        assert thinking == "reasoning only"
        assert content == ""
