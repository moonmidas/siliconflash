...
class TestGetSamplingParams:
    ...
    def test_fast_dflash_profile_uses_greedy_defaults_when_request_omits_sampling(self):
        """Omitted sampling params should resolve to DFlash-safe defaults under the fast profile."""
        import tempfile
        from pathlib import Path
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            temp, top_p, top_k, _, min_p, _, _, _, _, _ = get_sampling_params(
                None, None, "test-model"
            )
            assert temp == 0.0
            assert top_p == 1.0
    ...
    def test_fast_dflash_profile_preserves_explicit_client_sampling(self):
        """Explicit client params should still win under the fast DFlash profile."""
        import tempfile
        from pathlib import Path
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            temp, top_p, top_k, _, min_p, _, _, _, _, _ = get_sampling_params(
                0.7, 0.8, "test-model", req_min_p=0.1,
            )
            assert temp == 0.7
            assert top_p == 0.8
            assert top_k == 0
    ...
    def test_fast_dflash_profile_streaming_chat_uses_omitted_bounds_for_thinking(self):
        import tempfile
        from pathlib import Path
        from omlx.server import _apply_fast_profile_omitted_chat_bounds
...
def test_extract_dflash_usage_fields_maps_unfused_wait_components():
    """Split unfused wait attribution telemetry should be exposed in usage."""
    output = MagicMock()
    output.backend_metadata = {
        "dflash": {
            ...
            "used": True,
            "reason": "ok",
            "backend": "mirror_sd_mlx",
            "telemetry": {
                "verify_eval_wait_unfused_target_posterior_steps": 9,
                "verify_eval_wait_unfused_target_posterior_mean_s": 0.22,
                "verify_eval_wait_unfused_draft_logits_steps": 9,
                "verify_eval_wait_unfused_draft_logits_mean_s": 0.07,
                "ddtree_tree_budget": 4,
                "ddtree_cycles_completed": 12,
                "ddtree_tokens_per_second": 123.5,
            },
        }
    }

    usage = _extract_dflash_usage_fields(output)

    assert usage["dflash_requested"] is True
    assert usage["dflash_used"] is True
    assert usage["dflash_backend"] == "mirror_sd_mlx"
    assert usage["dflash_verify_eval_wait_unfused_target_posterior_steps"] == 9
    assert usage["dflash_verify_eval_wait_unfused_target_posterior_mean_s"] == 0.22
    assert usage["dflash_verify_eval_wait_unfused_draft_logits_steps"] == 9
    assert usage["dflash_verify_eval_wait_unfused_draft_logits_mean_s"] == 0.07
    assert usage["dflash_ddtree_tree_budget"] == 4
    assert usage["dflash_ddtree_cycles_completed"] == 12
    assert usage["dflash_ddtree_tokens_per_second"] == 123.5
