...
class TestResponsesEndpoint:
    def test_responses_endpoint_uses_fast_profile_greedy_defaults_when_sampling_is_omitted(self, tmp_path):
        from omlx.server import app, _server_state

        state_dir = tmp_path / "response-state"
        engine = RecordingResponsesEngine(outputs=[MockGenerationOutput(text="Done.", finish_reason="stop")])
        engine.chat = AsyncMock(return_value=MockGenerationOutput(text="Done.", finish_reason="stop"))
...
class TestChatCompletionEndpoint:
    ...
    def test_chat_completion_exposes_dflash_usage_metadata(self, client, mock_llm_engine):
        mock_llm_engine.chat = AsyncMock(return_value=MockGenerationOutput(
            text="Chat response.",
            prompt_tokens=10,
            completion_tokens=5,
            backend_metadata={
                "dflash": {
                    "requested": True,
                    "used": False,
                    "reason": "DFlash streaming is not yet supported",
                    "backend": "bstnxbt_mlx",
                }
            },
        ))
        ...
        assert usage["dflash_used"] is False
        assert usage["dflash_reason"] == "DFlash streaming is not yet supported"
        assert usage["dflash_backend"] == "bstnxbt_mlx"

    def test_chat_completion_uses_fast_profile_greedy_defaults_when_sampling_is_omitted(self, client, mock_llm_engine):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            _server_state.settings_manager = manager
            mock_llm_engine.chat = AsyncMock(return_value=MockGenerationOutput(text="Chat response."))
            ...
            assert response.status_code == 200
            _, kwargs = mock_llm_engine.chat.call_args
            assert kwargs["temperature"] == 0.0
            assert kwargs["top_p"] == 1.0
            assert kwargs["top_k"] == 0
            assert kwargs["min_p"] == 0.0
    ...
    def test_streaming_chat_completion_uses_fast_profile_omitted_bounds_for_thinking(self, client, mock_llm_engine):
        from omlx.server import _server_state
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            response = client.post(
                ...
                json={
                    "model": "test-model",
                    "messages": [{"role": "user", "content": "nice"}],
                    "stream": True,
                },
            )
    ...
    def test_chat_completion_preserves_explicit_non_greedy_sampling_under_fast_profile(self, client, mock_llm_engine):
        from omlx.server import _server_state
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            mock_llm_engine.chat = AsyncMock(return_value=MockGenerationOutput(
                ...
                backend_metadata={
                    "dflash": {
                        "requested": True,
                        "used": False,
                        "reason": "DFlash currently supports greedy decoding only (temperature must be 0)",
                        "backend": "bstnxbt_mlx",
                    }
                },
            ))
            ...
            usage = response.json()["usage"]
            assert usage["dflash_used"] is False
            assert usage["dflash_reason"] == "DFlash currently supports greedy decoding only (temperature must be 0)"
        _server_state.settings_manager = original_settings_manager
...
class TestAnthropicMessagesEndpoint:
    ...
    def test_anthropic_messages_use_fast_profile_greedy_defaults_when_sampling_is_omitted(self, client, mock_llm_engine):
        from omlx.server import _server_state
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            _server_state.settings_manager = manager
            mock_llm_engine.chat = AsyncMock(return_value=MockGenerationOutput(text="Anthropic response."))
