...
class TestSchedulerStopTokens:
    ...
    def test_get_stop_tokens_can_ignore_eos(self, mock_model, mock_tokenizer):
        scheduler = Scheduler(model=mock_model, tokenizer=mock_tokenizer)

        stop_tokens = scheduler._get_stop_tokens(ignore_eos=True)

        assert mock_tokenizer.eos_token_id not in stop_tokens
