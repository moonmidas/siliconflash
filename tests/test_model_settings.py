...
from omlx.model_settings import (
    ...
    apply_dflash_fast_model_settings,
)
...
def test_apply_dflash_fast_model_settings_enables_greedy_defaults_and_thinking():
    updated = apply_dflash_fast_model_settings(
        ModelSettings(temperature=0.7, top_p=0.9, top_k=40, min_p=0.1, force_sampling=True),
        draft_model="z-lab/Qwen3.5-9B-DFlash",
        enable_thinking=True,
        thinking_budget=256,
    )

    assert updated.dflash_enabled is True
    assert updated.dflash_draft_model == "z-lab/Qwen3.5-9B-DFlash"
    assert updated.dflash_use_mlx_native_drafter is True
    assert updated.temperature == 0.0
    assert updated.top_p == 1.0
    assert updated.top_k == 0
    assert updated.min_p == 0.0
    assert updated.force_sampling is False
    assert updated.chat_template_kwargs == {"enable_thinking": True}
    assert updated.forced_ct_kwargs == ["enable_thinking"]
    assert updated.thinking_budget_enabled is True
    assert updated.thinking_budget_tokens == 256
    assert updated.dflash_auto_profile is True


def test_apply_dflash_fast_model_settings_can_remove_forced_thinking():
    updated = apply_dflash_fast_model_settings(
        ModelSettings(
            chat_template_kwargs={"enable_thinking": True, "other": 1},
            forced_ct_kwargs=["enable_thinking", "other"],
    ...
    assert updated.chat_template_kwargs == {"other": 1}
    assert updated.forced_ct_kwargs == ["other"]
    assert updated.thinking_budget_enabled is False
    assert updated.thinking_budget_tokens is None
    assert updated.dflash_auto_profile is True
