...
class TestUsageExtendedFields:
    ...
    def test_usage_with_timing(self):
        usage = Usage(
            ...
            dflash_used=False,
            dflash_reason="DFlash streaming is not yet supported",
            dflash_backend="bstnxbt_mlx",
        )
    ...
    def test_usage_none_fields_excluded(self):
        ...
        assert "dflash_requested" not in dumped
        assert "dflash_used" not in dumped
        assert "dflash_reason" not in dumped
        assert "dflash_backend" not in dumped
...
class TestUsageChunkFormat:
    ...
    def test_usage_chunk_with_dflash_fields(self):
        chunk = ChatCompletionChunk(
            id="chatcmpl-test",
            model="test-model",
            choices=[],
        ...
        data = json.loads(chunk.model_dump_json(exclude_none=True))
        assert data["usage"]["dflash_requested"] is True
        assert data["usage"]["dflash_used"] is True
        assert data["usage"]["dflash_backend"] == "bstnxbt_mlx"
